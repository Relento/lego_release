...
def lego_decode_bid(heat, rot, bids_all, masks, reg=None, trans=None, predict_bid=None):
    batch, cat, height, width = heat.size()
    ...
    heat = _nms(heat)
    if heat.shape[1] != 1:
        heat = torch.softmax(heat, dim=1)

    from collections import Counter
    detections = []
    for i in range(batch):
        bids = bids_all[i][masks[i].bool()].detach().cpu().tolist()
        bids_ct = Counter(bids)
        batch_detections = []
        for bid, ct in bids_ct.items():
            h = heat[i, bid, :, :].reshape(-1)
            this_scores, this_inds = torch.topk(h, k=ct)
            ys = (this_inds / width).int().float()
            xs = (this_inds % width).int().float()
            if reg is not None:
                xs += reg.reshape(batch, 2, -1)[i, 0, this_inds]
                ys += reg.reshape(batch, 2, -1)[i, 1, this_inds]
            else:
                xs += 0.5
                ys += 0.5
            if rot is not None:
                this_rot = rot.reshape(batch, rot.size(1), -1)[i, :, this_inds].argmax(0)

            if predict_bid is not None:
                this_bid = predict_bid.reshape(batch, predict_bid.size(1), -1)[i, :, this_inds].argmax(0)

            if trans is not None:
                this_trans = trans.reshape(batch, 3, -1)[i, :, this_inds].permute(1, 0)
            ...
            if predict_bid is None:
                clses = torch.ones_like(this_scores) * bid
            else:
                clses = this_bid
            to_cat = [kps, this_scores[:, None], clses[:, None]]
            if rot is not None:
                to_cat.append(this_rot[:, None])
            if trans is not None:
                to_cat.append(this_trans)
            this_detections = torch.cat(to_cat, dim=-1)
            batch_detections.append(this_detections)
        batch_detections = torch.cat(batch_detections, dim=0)
        detections.append(batch_detections)
